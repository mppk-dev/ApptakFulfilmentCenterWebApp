using System;
using System.Collections.Generic;

namespace ApptakFulfilmentCenterWebApp.Models;

public partial class SmItemMaster
{
    public string? ActCode { get; set; }

    public string? PrdCode { get; set; }

    public string? PrdName { get; set; }

    public string? PrdSize { get; set; }

    public decimal? NewCost { get; set; }

    public decimal? NewTrade { get; set; }

    public decimal? OldTrade { get; set; }

    public string? AccountGroup { get; set; }

    public string? SalesGroup { get; set; }

    public string? CompleteStop { get; set; }

    public string? MajorStop { get; set; }

    public string? MiniStop { get; set; }

    public decimal? Lenght { get; set; }

    public decimal? Breadth { get; set; }

    public decimal? Hieght { get; set; }

    public decimal? Carton1 { get; set; }

    public decimal? TheropaticGroup { get; set; }

    public string? StorageCondition { get; set; }

    public decimal? Carton2 { get; set; }

    public decimal? Carton3 { get; set; }

    public decimal? Carton4 { get; set; }

    public decimal? STax1 { get; set; }

    public decimal? STax2 { get; set; }

    public string? PrdGroup { get; set; }

    public decimal? NewMrsp { get; set; }

    public decimal? OldMrsp { get; set; }

    public decimal? STax3 { get; set; }

    public decimal? OldCost { get; set; }

    public string? Division { get; set; }

    public decimal? PrdWieght { get; set; }

    public string? PrdType { get; set; }

    public string? STaxFlag { get; set; }

    public string? Prod1 { get; set; }

    public string? Prod1Desc { get; set; }

    public string? Prod2 { get; set; }

    public string? Prod2Desc { get; set; }

    public string? Prod3 { get; set; }

    public string? Prod3Desc { get; set; }

    public string? Prod4 { get; set; }

    public string? Prod4Desc { get; set; }

    public string? Prod5 { get; set; }

    public string? Prod5Desc { get; set; }

    public string? Prod6 { get; set; }

    public string? Prod6Desc { get; set; }

    public string? Prod7 { get; set; }

    public string? Prod7Desc { get; set; }

    public string? Prod8 { get; set; }

    public string? Prod8Desc { get; set; }

    public string? Prod9 { get; set; }

    public string? Prod9Desc { get; set; }

    public int? GstRegistered { get; set; }

    public int? GstNregistered { get; set; }
}
